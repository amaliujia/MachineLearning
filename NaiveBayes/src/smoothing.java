...
public class smoothing {
    ...
    public static void main(String[]args){
        ...
        smoothing naive = new smoothing();
        naive.smm = Double.parseDouble(args[2]);
        try {
            naive.train(Train);
            naive.test(Test);
        } catch (IOException e) {
    ...
    private int bayse(String instance) throws IOException {
        ...
        while ((line = scanner.readLine()) != null){
            ...
            if(pMap1.containsKey(line)){
                ...
                // pp1 += Math.log(((1.0) / (tokens1 * 1.0 + (totalVal) * 1.0 * smm )));
            }

            if(pMap2.containsKey(line)){
                pp2 += pMap2.get(line); //Math.log(pMap2.get(line));
            }else {
                // pp2 += Math.log((1.0) / (tokens2 * 1.0 + smm * (totalVal) * 1.0));
            }
    ...
    private void train(String trainPath) throws IOException {
        ...
        while (iterator.hasNext()){
            ...
            if(frqMap1.containsKey(s)){
                int n = frqMap1.get(s);
                double p = Math.log((n * 1.0 + smm) / (tokens1 * 1.0 + smm * val));
                pMap1.put(s, p);
            }else{
                int n = 0;
                double p =  Math.log((n * 1.0 + smm) / (tokens1 * 1.0 + smm * val));
            ...
            if(frqMap2.containsKey(s)){
                int n = frqMap2.get(s);
                double p =  Math.log((n * 1.0 + smm) / (tokens2 * 1.0 + smm * val));
                pMap2.put(s, p);
            }else{
                int n = 0;
                double p =  Math.log((n * 1.0 + smm) / (tokens2 * 1.0 +  smm * val));
